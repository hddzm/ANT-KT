...
def main():
    ...
    config.device_ids = [0]  # 因为设置了 CUDA_VISIBLE_DEVICES，所以这里的 0 实际上指的是物理设备 5
...
def train_student(config, fold_path):
    ...
    if config.pre_train_path is not None:
        optimizer = NoamOpt(config.embed_size, 1, config.warmup_steps, torch.optim.Adam(model.parameters(),
                                                                                       lr=config.lr, weight_decay=config.wd), step=config.warmup_steps)
    else:
        optimizer = NoamOpt(config.embed_size, 1, config.warmup_steps, torch.optim.Adam(model.parameters(),
                                                                                       lr=config.lr, weight_decay=config.wd))
    if config.use_adamw:
        optimizer = torch.optim.AdamW(model.parameters(), lr=3e-4, weight_decay=config.wd)

    # define training loop
    best_AUC = 0.0
    effect = initialize_effect()
    input_settings = [model, optimizer, config]
    ...
    for epoch in range(config.epochs):
        ...
        if config.evalmodel == 'weight-sharing':
            training_epoch_avg_loss, epoch_acc, epoch_auc = train_super(input_settings, train_dataloader)
        else:
            training_epoch_avg_loss, epoch_acc, epoch_auc = train(input_settings, train_dataloader)

        effect['train_loss'].append(training_epoch_avg_loss)
        ...
        # early stopping?
        if config.early_stop and epoch > config.early_stop_num:
